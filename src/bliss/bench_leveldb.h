   ...
   public:
    ...
    bool get(KEY_TYPE key) override {
        ...
        leveldb::Status status =
            _db->Get(leveldb::ReadOptions(), key_str, &value);
        return status.ok();
    }
    ...
    void put(KEY_TYPE key, VALUE_TYPE value) override {
        ...
        leveldb::Status status =
            _db->Put(leveldb::WriteOptions(), key_str, value_str);
        if (!status.ok()) {
            std::cerr << "LevelDB put error: " << status.ToString()
                      << std::endl;
        }
    }

    void end_routine() override { delete _db; }
